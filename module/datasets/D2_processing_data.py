...
class Pty2Rating:
    ...
    def download(self):
        ...
        rating_file = os.path.join(self.root_path, "processing", "D2-pty2ratings.xlsx")
        trust_file = os.path.join(self.root_path, "processing", "D2-pty2trust.xlsx")
        mistrust_file = os.path.join(self.root_path, "processing", "D2-pty2mistrust.xlsx")
    ...
    def processing_ratings(self):
        ...
        user_ratings_dataset = pd.read_excel(self.user_rating_data_path, sheet_name="sheet1")
        # set up the max row index
        max_row_index = max(set(user_ratings_dataset['user']))
        # Only 1508 items remain rated
        user_ratings_dataset = user_ratings_dataset.drop(
            user_ratings_dataset[user_ratings_dataset['user'] > 1508].index, axis=0)
        user_ratings_dataset = user_ratings_dataset.drop(
            user_ratings_dataset[user_ratings_dataset['movie'] > 1508].index, axis=0)
        # Build user item rating pivot table
        user_item_pivot = pd.pivot_table(user_ratings_dataset[['user', 'movie', 'rating']],
                                         columns=['movie'],
                                         index=['user'],
                                         values="rating",
                                         fill_value=0)
        ...
        user_item_pivot = user_item_pivot.combine_first(pd.DataFrame(
            ...
            index=[i for i in range(1, max_row_index + 1)]
        ))
        ...
        drop_row_index_list = list(count_row_nonzero[count_row_nonzero <= 130].index)
        drop_col_index_list = list(count_col_nonzero[count_col_nonzero <= 150].index)
        ...
        user_item_expand = pd.DataFrame(
            ...
            columns=[i for i in
                     range(user_item_pivot.columns[-1] + 1, user_item_pivot.columns[-1] + 1 + (num_row - num_col))])
        user_item_pivot = pd.concat([user_item_pivot, user_item_expand], axis=1)
        # Export data to excel sheet
        user_item_pivot.to_excel(os.path.join(self.root_path, "processing", "D2-pty2ratings.xlsx"),
                                 index=False,
                                 header=False)
    ...
    def processing_trust(self, drop_row_index_list):
        ...
        # Export data to excel sheet
        user_trust_dataset.to_excel(os.path.join(self.root_path, "processing", "D2-pty2trust.xlsx"),
    ...
    def processing_mistrust(self, user_trust_dataset):
        ...
        # Export data to excel sheet
        user_mistrust_dataset.to_excel(os.path.join(self.root_path, "processing", "D2-pty2mistrust.xlsx"),
                                       index=False, header=False)
