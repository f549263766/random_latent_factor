...
class Pty4Rating:
    ...
    def download(self):
        ...
        rating_file = os.path.join(self.root_path, "processing", "D4-pty4ratings.xlsx")
        trust_file = os.path.join(self.root_path, "processing", "D4-pty4trust.xlsx")
        mistrust_file = os.path.join(self.root_path, "processing", "D4-pty4mistrust.xlsx")
    ...
    def processing_ratings(self):
        ...
        user_ratings_dataset = pd.read_excel(self.user_rating_data_path, sheet_name="D4-movie-ratings")
        ...
        user_ratings_dataset = user_ratings_dataset.drop(
            user_ratings_dataset[user_ratings_dataset['movie'] > 951].index, axis=0)
        # Build user item rating pivot table
        user_item_pivot = pd.pivot_table(user_ratings_dataset[['user', 'movie', 'ratings']],
                                         columns=['movie'],
                                         index=['user'],
                                         values="ratings",
                                         fill_value=0)
        ...
        drop_row_index_list = list(count_row_nonzero[count_row_nonzero <= 50].index)
        drop_col_index_list = list(count_col_nonzero[count_col_nonzero <= 50].index)
        ...
        user_item_pivot.to_excel(os.path.join(self.root_path, "processing", "D4-pty4ratings.xlsx"),
                                 index=False,
                                 header=False)
    ...
    def processing_trust(self, drop_row_index_list):
        ...
        # Export data to excel sheet
        user_trust_dataset.to_excel(os.path.join(self.root_path, "processing", "D4-pty4trust.xlsx"),
    ...
    def processing_mistrust(self, user_trust_dataset):
        ...
        # Export data to excel sheet
        user_mistrust_dataset.to_excel(os.path.join(self.root_path, "processing", "D4-pty4mistrust.xlsx"),
                                       index=False, header=False)
