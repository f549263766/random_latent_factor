...
class PtyModel:
    ...
    def fix(self, x, y, x_co, y_co, x_mistrust, y_mistrust):
        # beta-step
        ...
        # h-step
        self.h = np.dot((np.dot(x, self.beta.T) + np.dot(x_co, self.gamma.T) + np.dot(x_mistrust, self.mu.T)),
                        np.linalg.inv(np.dot(self.beta, self.beta.T) + np.dot(self.gamma, self.gamma.T) +
                                      np.dot(self.mu, self.mu.T) + np.asarray(1e-5)))
